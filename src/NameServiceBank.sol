...
contract NAME_SERVICE_BANK is Test {
    ...
    function setUsername(
        ...
        // console.logBytes(bytes(usernameOf[msg.sender]));

        // obfuscate the name
        for (uint256 i; i < obfuscationDegree; ++i) {
            bytes(usernameOf[msg.sender]).push(); // @note this changes the value of usernameOf[msg.sender] which means msg.sender has a different username now
        }
